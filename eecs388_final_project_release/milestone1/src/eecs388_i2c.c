...
void set_up_I2C(){
    ...
    success = metal_i2c_transfer(i2c,PCA9685_I2eC_ADDRESS,bufWrite,1,bufRead,1);//initial read
...
void steering(int angle){
    int valToBreak = getServoCycle(angle); // return (int) circle value
    breakup(valToBreak, &bufWrite[3], &bufWrite[4]); 
    bufWrite[0] = PCA9685_LED0_ON_L+4;
    bufWrite[1] = bufWrite[3]; 
    bufWrite[2] = bufWrite[4];
...
void driveForward(uint8_t speedFlag){
    unit8_t data_low = 0;
    unit8_t data_high = 0;
    breakup(speedFlag, &data_low, &data_high);
    bufWrite[0] = 0x0C;
    bufWrite[1] = data_low;
    bufWrite[2] = data_high;
    metal_i2c_transfer(i2c,PCA9685_I2C_ADDRESS,bufWrite,5 ,bufRead,1);
